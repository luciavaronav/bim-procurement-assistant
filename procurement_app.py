...
def _read_raw_csv(uploaded_file) -> pd.DataFrame:
    """Read CSV safely regardless of column count inconsistencies."""
    import io as _io
    ...
    max_cols = max((len(line.split(",")) for line in lines if line.strip()), default=5)
    df = pd.read_csv(_io.StringIO(content), header=None,
                     names=range(max_cols), dtype=str, engine="python")
    return df


def _detect_schema(df_raw: pd.DataFrame) -> str:
    """
    Returns 'revit' if Family/Type/Panel Name/Level/Count schema,
    returns 'gurtz' if Level/Location/Panel Name/Type panelboard schema.
    """
    for i in range(min(20, len(df_raw))):
        row = [str(v).strip().lower() for v in df_raw.iloc[i].fillna("")]
        if row[0] == "family":
            return "revit"
        if "level" in row and "panel name" in row:
            return "gurtz"
    return "revit"  # default


def _parse_revit(df_raw: pd.DataFrame):
    """Parse standard Revit Electrical Equipment Schedule (Family/Type/Panel Name/Level/Count)."""
    header_row_idx = None
    for i in range(min(20, len(df_raw))):
        row_vals = [str(v).strip().lower() for v in df_raw.iloc[i].fillna("")]
        if row_vals[0] == "family":
            header_row_idx = i
            break

    if header_row_idx is None:
        st.error("❌ Could not find 'Family' header row in first 20 rows.")
        st.stop()

    df = df_raw.iloc[header_row_idx + 1:, :5].copy()
    ...
    # Remove subtotals
    mask_sub = (df["Type"] == "") & (df["Panel Name"] == "") & (df["Level"] == "")
    df = df[~mask_sub & (df["Family"] != "")].copy()
    return df


def _parse_gurtz(df_raw: pd.DataFrame):
    """
    Parse Gurtz/NECA multi-header panelboard schedule.
    Each row = one panel (Count = 1). Maps to standard 5-column schema.
    """
    # Find the header row containing 'Level'
    header_row_idx = None
    col_map = {}
    for i in range(min(20, len(df_raw))):
        row = df_raw.iloc[i].fillna("").astype(str).tolist()
        row_lower = [v.strip().lower() for v in row]
        if "level" in row_lower:
            header_row_idx = i
            # Map key column names to positions
            for j, val in enumerate(row_lower):
                if val == "level":         col_map["Level"] = j
                if val == "panel name":    col_map["Panel Name"] = j
                if val == "type":          col_map["Type"] = j
                if val == "location":      col_map["Location"] = j
            break

    if header_row_idx is None:
        st.error("❌ Could not find 'Level' header row in Gurtz-format CSV.")
        st.stop()

    df_data = df_raw.iloc[header_row_idx + 1:].reset_index(drop=True)
    level_col = col_map.get("Level", 0)
    panel_col = col_map.get("Panel Name", 3)
    type_col  = col_map.get("Type", 5)

    rows = []
    for _, row in df_data.iterrows():
        level = str(row[level_col]).strip() if pd.notna(row[level_col]) else ""
        panel = str(row[panel_col]).strip() if pd.notna(row[panel_col]) else ""
        typ   = str(row[type_col]).strip()  if pd.notna(row[type_col])  else ""
        # Skip blank/separator rows and header-repeat rows
        if level.lower() in ["level", "nan"] or (panel == "" and typ == ""):
            continue
        rows.append({
            "Family": "Panelboard",
            "Type": typ,
...
def clean_bim_csv(uploaded_file):
    """
    Auto-detecting cleaning pipeline.
    Supports: Revit Electrical Equipment Schedule & Gurtz/NECA panelboard format.
    Returns (df_clean, df_family, df_issues, total_units, schema_name).
    """
    df_raw = _read_raw_csv(uploaded_file)
    schema = _detect_schema(df_raw)

    if schema == "gurtz":
        df_clean = _parse_gurtz(df_raw)
        schema_label = "Gurtz/NECA Panelboard Schedule"
    else:
        df_clean = _parse_revit(df_raw)
        schema_label = "Revit Electrical Equipment Schedule"

    # Issues audit (same for both schemas)
    def get_issues(row):
        errors = []
        if not str(row["Type"]).strip():       errors.append("Type undefined")
        if not str(row["Panel Name"]).strip(): errors.append("Panel not assigned")
        if not str(row["Level"]).strip():      errors.append("Level missing")
        return "; ".join(errors)

    df_clean["Issues"] = df_clean.apply(get_issues, axis=1)
    df_issues = df_clean[df_clean["Issues"] != ""].copy()

    # For Gurtz (single family), group by Level for meaningful summary
    if schema == "gurtz":
        df_family = (df_clean[df_clean["Level"].str.strip() != ""]
                     .groupby("Level")["Count"].sum()
                     .reset_index()
                     .rename(columns={"Level": "Family"})
...
if run_btn and uploaded_file:
    ...
    st.info(f"📋 Schema detected: **{schema_label}** — {total_units} units, {len(df_clean)} BOM lines")
    ...
    with tab4:
        ...
        if "gurtz" in schema_label.lower():
            st.caption(
                "ℹ️ This dataset contains a single equipment type (Panelboard). "
                "Lead times are uniform across all floors (6–12 weeks). "
                "For mixed-equipment schedules, lead times vary by equipment family."
            )
